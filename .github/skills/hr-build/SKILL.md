---
name: hr-build
description: 用户显式调用 /hr-build 时，使用 Human ReAct build task 执行明确授权的工作或检查现实，并验证结果。
user-invocable: true
...
本入口将用户的显式调用解释为选择 `build`，命令后的文字是本轮请求。保留对象、上下文和具体操作限制；本轮及续轮遵循所选 Task，只有用户明确重新选择才切换，不自动运行下一 Task。Skill 不切换宿主运行模式，也不增加工具或操作权限。
...
2. [Build Task](../../../.human-react/tasks/build.md)
3. [公共输出模板](../../../.human-react/templates/common.md)
4. [Build 输出模板](../../../.human-react/templates/build.md)
