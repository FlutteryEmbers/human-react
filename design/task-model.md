...
校准的价值取决于是否改变并改善解释，而非收集了多少回答。问题明确时直接解释；未反馈也不要求完成问卷或证明掌握。Copilot 的可选面板只是这种交互的宿主实现，与 Shape、Plan 中处理重要 Human-owned decision 的交互保持独立。
...
Review 将已知独立取证合批，并在每批后判断继续调查是否可能改变具体结论、证据边界或必要决定，以减少同一 task 中 harness 的无效模型—工具往返。依赖调查仍顺序执行，关键缺口和反证不能为省调用而跳过；证据充分后直接交付，不增加固定自检轮。批次是调度意图，不等同于并行工具数或实际模型调用数，协议不能保证宿主减少调用，也不承诺固定 token 节省比例。
...
模糊评价需要先形成有来源的判断依据。Review 区分明确规范、目标推导和暂定标准，避免以现状证明应然。标准改变会影响结论时保留条件分支；结论稳定时直接给出有边界的判断，避免把每个评价维度变成审批。条件分析不制造已成立的缺陷。
...
可判断性还取决于 Human 是否理解候选后果。Shape 将必要背景与场景中的成本、收益和影响放在候选附近，帮助用户形成偏好；理解后修正偏好是正常建模进展，不要求先走 Orient 或重启讨论。标准和推荐仍不等于 Human 承诺。
...
验收依据连接目标与执行验证。Plan 区分有来源的验收条件和 Agent 建议门槛，避免把“更快”等表达擅自转成硬性数值，再据此制造修改范围。明确目标下的普通验证方法仍可自主选择，不引入逐项确认。
...
自动推进依赖问题理解有依据，而不仅是框架内部的推理连贯。Core 在工作理解形成、重要信息变化和方向收敛时检查明确限定与探索性举例、原目标与手段的区别、关键遗漏以及暂定状态的来源；不要求每轮列替代项，也不向所有 task 授予独立重设计能力。

Shape 允许修正议题本身，用 Focus 和 Model Delta 说明与原诉求的关系。Plan 在关闭重要选择前核对依据及委托边界，不能用 Agent 自提候选与自定标准相互支持来替用户确定目标，也不能用风险披露代替风险接受。可撤回的操作仍可能包含不属于 Agent 的价值选择。

交互成本按决定的实际影响控制：证据与委托充分时自主处理；无重要承诺的暂定理解披露后继续；重要 Human-owned 分歧先询问，再处理依赖它的部分。Copilot 的共享适配使 Shape 与 Plan 可以把已识别的决定转成面板交互，分歧不必是逻辑冲突。已回答事项归对应 Context，最终 Human Attention 可为 none；成功指标是避免重要框架偏差与决策越界，并保持普通工作自动推进。
